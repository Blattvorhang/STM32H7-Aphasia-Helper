...
void OV7670_colorbar_test (void)
 {
	SCCB_WR_Reg(0x71, 0x80);
 }

void OV_EXTI_Control(FunctionalState State)
{
    static GPIO_InitTypeDef GPIO_Initure = {.Pull = GPIO_PULLUP, .Speed = GPIO_SPEED_FREQ_HIGH, .Pin = OV7670_VSYNC_Pin};
    ...
    HAL_GPIO_Init(OV7670_VSYNC_GPIO_Port, &GPIO_Initure);        
}
